...
#define AUTHOR ".sρiηX҉.serfreeman1337" // ICQ: 50429042
#define MAX_IP_LENGTH              16
#define MAX_NAME_LENGTH            32
#define MAX_RESOURCE_PATH_LENGTH   64
#define MAX_USER_INFO_LENGTH       256
#define MAX_MENU_LENGTH            512
#define LASTUPDATE  "02, August (08), 2021"
...
enum _:playersDataStruct {
    ...
    PLAYER_KILLS,
    PLAYER_NEEDKILLS,
    PLAYER_WEAPONNAME[MAX_NAME_LENGTH],
    PLAYER_RANKPOS,
    PLAYER_ICON[MAX_NAME_LENGTH],
    PLAYER_TEAM,
...
enum _:weaponSetStruct {
    WSET_SHOWNAME[MAX_NAME_LENGTH],
    WSET_KILLS,
    WSET_SKIP,
    WSET_ICONSPRITE[MAX_NAME_LENGTH],
...
new cvar[cvars]
new g_mp_trick, g_nextmap
new nextMap[MAX_NAME_LENGTH],nextMapBuffer[MAX_NAME_LENGTH]
...
new playersData[MAX_NAME_LENGTH + 1][playersDataStruct]
...
// leet sound
new sound_winner[MAX_RESOURCE_PATH_LENGTH],sound_levelup[MAX_RESOURCE_PATH_LENGTH],sound_leveldown[MAX_RESOURCE_PATH_LENGTH],prolevel_music[MAX_RESOURCE_PATH_LENGTH]
...
public plugin_precache(){
    ...
    new winnerSnd[MAX_USER_INFO_LENGTH]
...
public plugin_init(){
    ...
    g_nextmap = get_cvar_string("amx_nextmap",nextMapBuffer,charsmax(nextMapBuffer))
    //bind_pcvar_string(g_nextmap, nextMapBind,charsmax(nextMapBind))

    if ( !cstrike_running() || (is_running("dod") != 1)  )
        set_pcvar_string(g_mp_trick,"") //RTV push. Mods without rounds etc go back to mapcycle regardless of vote outcomes.
...
public plugin_cfg(){
    server_print "^n   Half-Life GunGame Copyright (c) 2021 %s",AUTHOR
    server_print "   Version %s build on %s^n", VERSION, LASTUPDATE
...
public CMD_GGToggle(id,level,cid){
    ...
    new adminName[MAX_NAME_LENGTH],adminAuth[MAX_AUTHID_LENGTH],adminIp[MAX_IP_LENGTH]
...
public Enable_GunGame(){
    ...
    if(!cvarState){
        new players[MAX_NAME_LENGTH],pnum
...
public load_cfg(){
    new mapName[MAX_NAME_LENGTH],cfgFilePath[MAX_MENU_LENGTH],cfgFileLen,bool:isMapCfg
        ...
        while(!feof(f)){
            ...
            switch(currentBlockId){
                ...
                case CFG_BLOCK_CVARS:{ // parse cvar block
                    ...
                    new cvarName[MAX_NAME_LENGTH + 8],cvarValue[MAX_USER_INFO_LENGTH],cvarId
                ...
                case CFG_BLOCK_MODSETTINGS:{ // parse mod settings
                    ...
                    switch(modBlockId){
                        ...
                        case CFG_MOD_WEAPONLIST:{
                            ...
                            new weaponName[MAX_NAME_LENGTH],szData[4][10]
...
Item_SetLock(classname[MAX_NAME_LENGTH],ent,bool:lock = true,bool:special = false){
    if(!mapItems)
        mapItems = TrieCreate()
...
Item_LockCheck(ent){
    new classname[MAX_NAME_LENGTH],checkClassName[MAX_NAME_LENGTH]
    entity_get_string(ent,EV_SZ_classname,classname,charsmax(classname))
...
public Parse_WeaponSets(buffer[],lineCount,Trie:keyTrie,&Trie:hamHooks,bool:warmUp){
    ...
    switch(setBlockId){
        ...
        case CFG_LEVEL_MAIN:{ // parse level vars
            ...
            new keyName[20],keyValue[MAX_NAME_LENGTH + 8]
            strtok(buffer,keyName,charsmax(keyName),keyValue,charsmax(keyValue),'=',1)
            ...
            if(!data[0]){
                ...
                if(!isAG){
                    ...
                    for(new i,player ; i < pnum ; ++i){
                        ...
                        new pin_type = get_cvar_num("sv_hook") ? FL_WORLDBRUSH : FL_FROZEN //testing if less worldspawn sticks!?
                        set_pev(player,pev_flags,pev(player,pev_flags) | pin_type)
                    }
                }
            }
        }
...
public On_PlayerKilled(victim,killer){
    ...
    if(killer == inflictor){ // player killed by hitscan weapon
        ...
        new infClassname[MAX_NAME_LENGTH]
...
public On_TraceAttack(victim,attacker){
    ...
    static weaponName[MAX_NAME_LENGTH],weaponId
...
public Equip_PlayerWithWeapon(const id){
    if(is_user_alive(id) && ggActive) //[HAMSANDWICH] Entity has null private data (1)
    {
        ...
        if(playersData[id][PLAYER_CURRENTLEVEL] != playersData[id][PLAYER_LASTLEVEL] && warmUpMode == 2){
            ...
            if(!playersData[id][PLAYER_BOT] && !is_running("gearbox"))
        ...
        if(!playersData[id][PLAYER_NOREFIL] && weaponSet[WSET_INFARRAY]){
            new inflictor[MAX_NAME_LENGTH],inflictorAmmo[weaponAmmoStruct]
            ...
            new inflictor[MAX_NAME_LENGTH],inflictorData[4]
...
public gg_notify_msg(id,notifyId){
    ...
    new players[MAX_NAME_LENGTH],pnum,name[MAX_NAME_LENGTH],message[128],len
            ...
            case NOTIFY_TEAMUP,NOTIFY_TEAMWIN,NOTIFY_TEAMLAST: { // team messages
                ...
                if(get_user_team(player) == teamId) // team realted message
                    ...
                    if(!playersData[player][PLAYER_AGHL])
                        ...
                        new teamNameClr[MAX_IP_LENGTH + 6]
...
gg_get_level_playersnum(level){
    new players[MAX_NAME_LENGTH],pnum,count = -1
    get_players(players,pnum)
...
public Inflictors_SpawnHandler(ent){
    ...
    new entClassname[MAX_NAME_LENGTH],inflictorData[2]
...
public Inflictors_DestroyHandler(ent){
    ...
    new classname[MAX_NAME_LENGTH],inflictorAmmo[weaponAmmoStruct]
    entity_get_string(ent,EV_SZ_classname,classname,charsmax(classname))
...
public Inflictors_DamageHandler(ent,inflictor,attacker){
    ...
    new classname[MAX_NAME_LENGTH],inflictorData[2],owner = entity_get_int(ent,EV_INT_iuser3)
    entity_get_string(ent,EV_SZ_classname,classname,charsmax(classname))
...
public start_map_vote(Float:delay){
    ...
    switch(get_pcvar_num(cvar[CVAR_MAPCHOOSER_TYPE])){
        ...
        case 2:{
            ...
            if(get_pcvar_num(g_mp_trick) != -1)
            set_pcvar_num(g_mp_trick, -1)
...
public goto_nextmap()
{
    ...
    { // force change by hlds
        ...
        {
            log_amx"Making 'game-end' ent and sending it."
